...
open class Repeater: Equatable {
    ...
    public enum Interval {
        ...
        internal var value: DispatchTimeInterval {
            ...
            case .seconds(let v): return .milliseconds(Int( Double(v) * Double(1_000)))
            case .hours(let v): return .seconds(v * 3_600)
            case .days(let v): return .seconds(v * 86_400)
            }
        }
    }
    ...
    public class func every(_ interval: Interval, count: Int? = nil, _ handler: @escaping Observer) -> Repeater {
        let mode: Mode = (count != nil ? .finite(count ?? 0) : .infinite)
    ...
    private func timeFired() {
        ...
        case .finite:
            ...
            self.remainingIterations = (self.remainingIterations ?? 0) - 1
            if (self.remainingIterations ?? 0) == 0 {
